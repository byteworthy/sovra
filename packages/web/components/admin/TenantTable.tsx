    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-xl p-6 max-w-sm w-full mx-4 shadow-xl">
        ...
        <div className="flex justify-end gap-3">
          <button
            onClick={onCancel}
            className="h-8 px-3 text-sm rounded-md border border-border hover:bg-zinc-800/40 transition-colors"
          >
            {cancelLabel}
          </button>
          <button
      ...
      <div className="border border-border rounded-xl overflow-hidden">
        ...
        {tenants.map((tenant, i) => (
          <motion.div
            ...
            className="grid grid-cols-[2fr_1fr_1fr_1fr_1.5fr_1fr_40px] gap-4 px-4 items-center h-[56px] border-b border-border last:border-b-0 hover:bg-zinc-900/40 transition-colors duration-100 relative"
          >
            ...
            <div>
              <span
                className={[
                  ...
                  tenant.status === 'suspended' ? 'text-amber-400' : 'text-green-400',
                ].join(' ')}
              >
            ...
            <div className="relative">
              <button
                ...
                className="h-7 w-7 flex items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-zinc-800/50 transition-colors"
              >
              ...
              {openMenu === tenant.id && (
                <div className="absolute right-0 top-8 z-20 bg-card border border-border rounded-lg shadow-xl py-1 min-w-[140px]">
                  <button
                    className="w-full text-left text-sm px-3 py-2 hover:bg-zinc-800/40 transition-colors"
                    onClick={() => setOpenMenu(null)}
                  >
                    View
                  </button>
                  <button
                    className="w-full text-left text-sm px-3 py-2 hover:bg-zinc-800/40 transition-colors text-amber-400"
                  ...
                  <button
                    className="w-full text-left text-sm px-3 py-2 hover:bg-zinc-800/40 transition-colors text-red-400"
