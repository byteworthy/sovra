...
human_verification:
  ...
  - test: "Run `pnpm --filter @agentforge/web run lint` from repo root"
  ...
  - test: "Run `pnpm --filter @agentforge/web run type-check` from repo root"
...
| `packages/worker/go.mod` | Go module definition | ✓ VERIFIED | `module github.com/agentforge/worker`, go 1.26.2, gin + pgx + grpc deps |
...
|------|----|-----|--------|---------|
| packages/web | @agentforge/shared | pnpm-workspace.yaml + package.json | ✓ WIRED | workspace:* dep in web package.json |
...
**Test:** From repo root: `pnpm install && pnpm --filter @agentforge/web run test`
...
**Test:** From repo root: `pnpm install && pnpm --filter @agentforge/web run type-check`
**Expected:** `tsc --noEmit` exits 0 with no output
**Why human:** node_modules not installed in verification environment

#### 3. ESLint (including card.ctsx impact)

**Test:** From repo root: `pnpm install && pnpm --filter @agentforge/web run lint`
